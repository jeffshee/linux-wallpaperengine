...
void audio_callback (void* userdata, uint8_t* streamData, int length) {
	...
	while (streamLength > 0 && driver->getApplicationContext ().state.general.keepRunning) {
	    ...
	    // mix the audio; a disabled audio state mutes the output (the
	    // buffer is pre-zeroed) while decoding keeps advancing, so
	    // re-enabling resumes at the current position
	    const auto& audioState = driver->getApplicationContext ().state.audio;
	    SDL_MixAudioFormat (
